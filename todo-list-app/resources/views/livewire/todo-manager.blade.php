...
new class extends Component {
    ...
    {
        return [
            'todos' => Auth::check() ? Auth::user()->todos()->get() : collect(),
        ];
    }
    ...
    <form wire:submit='createTodo' class="flex flex-col gap-4 mb-4">
        ...
        <div class="steps-block">
            ...
            <div class="flex flex-col gap-4 mb-4 px-4 py-4">
                ...
                <x-text-input wire:model="smallSteps.{{ $index }}.name" placeholder="Sub-Task name..." class="p-2 border rounded" />
                <textarea wire:model="smallSteps.{{ $index }}.description" placeholder="Sub-Task description..." class="p-2 border rounded"></textarea>
                <button type="button" wire:click="removeSmallStep({{ $index }})" class="bg-red-500 text-white px-2 py-1 rounded hover:bg-red-700">Remove</button>
            </div>
            @endforeach
        </div>

        <button type="button" wire:click="addSmallStep" class="w-full px-3 py-1 bg-green-500 text-white rounded hover:bg-green-700 transition">
            Add Step
        </button>
